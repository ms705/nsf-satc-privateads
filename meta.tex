...
{\small
\begin{itemize}
\item Design of the cryptographic protocol for setting up the system.  Here, instead of invoking general secure multi-party computation, we may be able to design a custom protocol.  
\item Adapting existing protocols for obtaining and using tokens for the use case; making and validating design choices regarding when tokens are issued, when they expire, whether they are generated locally (with multi-use rate-limited credentials~\cite{CC:chklm06}) or pre-loaded (with anonymous credentials light~\cite{CCS:ballys13}).
\item Key management: what happens if the client’s browser storage is erased and they need new keys?  How are their keys stored in the browser and protected?
\item How would a daily issuance of fresh authentication tokens to clients work at the scale of Meta?
...
\noindent \textbf{8/1/22 - 11/30/22} Work on the one-factor scenario: give a prototype implementation, and a cryptographic protocol for the setup.  Make reasonable design choices regarding parameters such as expiration dates, etc.

\noindent \textbf{12/1/22 - 2/28/23} Write a technical paper on the one-factor prototype and submit it for peer review.  Extend the prototype to a two-factor one.

\noindent \textbf{3/1/23 - 5/31/23}  Write the technical paper on the two-factor prototype and submit it for peer review.

\noindent \textbf{6/1/23 - 8/31/23}  Incorporating feedback on the paper, preparing and delivering technical talks.
